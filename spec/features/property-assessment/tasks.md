...
- [ ] T5: `assess/pictures.py`: the two images (`feat-013/AC-3`). The stored listing photograph from
...
- [ ] T7: `tests/test_assessment_dossier.py`: the dossier, the absences, the criteria and the
...
- [ ] T12: `tests/test_assessment_pass.py`: the request and the answer, against a fake client
      (`feat-013/AC-5`, `feat-013/AC-11`, `feat-013/AC-13`). Including an unusable answer leaving a
      previous assessment untouched.

## Storage

- [ ] T13: `store/schema.py`: the table an assessment lives in, and its migration (`feat-013/AC-6`,
...
- [ ] T14: `store/core.py`: writing and reading one, and reading whether one is current
...
- [ ] T15: `tests/test_assessment_store.py`: the annotation row is byte-identical across two
...
- [ ] T16: `tests/test_assessment_store.py`: a price change does not make an assessment stale, and a
...
- [ ] T16a: `tests/test_assessment_store.py`: passing on one property does not make any assessment
...
- [ ] T17: `api.py`: `assess`, over what is in play (`feat-013/AC-1`, `feat-013/AC-9`). Found by the
...
- [ ] T18: `api.py`: the count is reported before anything is asked (`feat-013/AC-1`). This is the
...
- [ ] T19: `api.py`: absent by default (`feat-013/AC-10`). With no model configured the pass reports
...
- [ ] T20: `api.py`: an assessment decides nothing (`feat-013/AC-7`). It sets no judgment, hides no
...
- [ ] T21: `api.py`: the pass records itself through the core's recorder (`feat-013/AC-17`). Visible
...
- [ ] T22: `tests/test_assessment_pass.py`: in play and nothing else, the bounded pass, the skipping
...
- [ ] T23: `search/definition.py`: `assess.model` on a saved search, off unless turned on
...
- [ ] T24: `cli/main.py`, `cli/render.py`: `homescout assess`, with `--search`, `--limit` and
      `--json` (`feat-013/AC-12`). A stable exit code, which is invariant 6.

- [ ] T25: `api.py`: reading one property's assessment, in the shape both surfaces read
      (`feat-013/AC-12`). The seam the browser will draw from when feat-010 gains the screen for it.

- [ ] T26: `tests/test_assessment_pass.py`, `tests/test_cli_operations.py`: the same pass through
      the core and through the command leaves one state, and the switch is honoured
      (`feat-013/AC-12`, `feat-013/AC-15`).

## Once it has run

- [ ] T27: A change against feat-010 so the browser can start a pass (`feat-013/AC-12`,
