...
- [ ] AC-18: An assessment records what counts FOR the property as well as what counts against it.
      ...
      **The same standard, for a stronger reason.** A flattering sentence is easier to generate than
      a critical one and harder to doubt on reading, so an uncheckable point is more dangerous here
      than an uncheckable concern rather than less. A list of pleasant adjectives about a house
      somebody is deciding whether to drive four hours to see is worse than no list at all.
      ...
      **No severity.** A concern carries one because "serious" changes what somebody does about it.
      ...
      number nobody acts on and one more thing for a model to invent.
...
- [ ] AC-19: A pass asks each property only what it is still missing. A property whose recorded
      ...
      **Which of the three a property gets is decided from what it is missing**, per property, and
      never from a flag somebody has to remember to pass. The pass that fills a gap in two hundred
      readings and the pass that reads ten new properties are the same command.
      ...
      **A narrow request is only ever added to a reading that still holds.** One whose facts have
      moved is read again in full, because that request carries the earlier account forward as
      context and doing so for a changed property would carry a description of the old house into a
      reading of the new one.

      **A narrow request is only added to a reading the configured model itself wrote.** An
      assessment carries one model's name, so one whose concerns came from one model and whose
      favourable points came from another cannot be labelled honestly: naming either makes the row
      claim work it did not do. Where they differ the property is read again in full, which is the
