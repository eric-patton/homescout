...
- [x] T-pass-1: `store/schema.py`: the table an operation records itself in, and its lines
...
- [x] T-pass-2: `store/schema.py`: the migration (`feat-001/AC-31`). This store is opened by a build
      that brings an older file forward, and the installation it has to bring forward is 200MB with
      seven runs in it.

- [x] T-pass-3: `store/core.py`, `store/models.py`: writing and reading one (`feat-001/AC-31`).
...
- [x] T-pass-4: `store/core.py`: a pass not touched recently enough reads as stopped without
...
- [x] T-pass-4a: `store/core.py`: every line and every failure is scrubbed of credentials on the
...
- [x] T-pass-5: `tests/test_store_schema.py`, `tests/test_store_history.py`: the lifecycle, the
