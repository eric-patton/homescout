...
- The same house appears on all three providers with different address formatting. The brief's own
...
- Normalized street plus ZIP is the primary key to attempt. Coordinates within roughly fifty metres
  confirm or break a tie. A parcel number, where a provider supplies one, is the strongest signal
...
Someone consolidating listings from several providers struggles to tell whether two rows describe
the same house because every provider formats addresses differently, some supply coordinates and
...
every merge inspectable and reversible, without ever destroying the provider rows the merge was
built from.
...
- **The same person six months later** (primary, differently): needs to look at a suspicious record
  and see exactly which provider rows produced it and why they were joined.

### Jobs to be done

- Recognize the same property across providers despite formatting differences.
...
- A property listed on all three providers appears once.
...
- Every canonical record can be traced back to the provider rows underneath it.

### Constraints

- Provider rows are never destroyed, edited, or hidden. Canonical records are built on top of them.
...
- Deduplicating rows within a single provider's single response, which the source adapters handle.
