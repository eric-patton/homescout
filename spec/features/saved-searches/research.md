...
- The unit a person tunes is not a query, it is a search: an area, a set of filters, a set of
  criteria, and a set of providers, kept together and refined over weeks.
- Geography is the part no provider gets right. Providers accept coarse forms (a city, a county, a
...
- The brief's answer is two-stage: query the coarse form a provider accepts, then test each result
...
Someone searching a region struggles to express where they will actually live because provider
...
- Combine that with ordinary filters and with the providers to ask.
...
- A property outside a drawn shape never appears in results, however coarse the provider query was.
- An invalid definition is caught before any provider is contacted.
