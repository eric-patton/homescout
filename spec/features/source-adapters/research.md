...
- The three providers differ in almost every dimension that matters: what geography they accept,
  which filters they will apply on their side, how many results they will return, and what they do
  when they dislike a caller.
- Sources break. A provider changing its markup or its response shape is the expected case, not the
...
several unrelated providers because each accepts a different query shape, applies a different
subset of filters on its own side, and enforces a different undocumented ceiling on results, which
results in either provider-specific special cases smeared through the whole tool or a
lowest-common-denominator query that drags back far more data than anyone wanted. A solution should
let each provider be described once, in one place, in terms of what it can and cannot do, and let
the rest of the tool work against that description, without ever making enough requests to get the
user blocked.

### Target users

- **The person running searches** (primary): wants listings, and wants a provider outage to cost
  them that provider's rows rather than the run.
- **Whoever adds the fourth source later** (secondary, and realistically the same person): needs
  adding a provider to mean writing one adapter, not editing the core.

### Jobs to be done

- Fetch every listing matching a query from one provider, respecting whatever ceiling it enforces.
- Declare honestly which parts of a query this provider will apply itself, so the rest can be
  applied locally rather than assumed.
- Stay under every provider's tolerance without being told to, by default.
- Fail in a way that costs one provider's contribution and nothing else.

### Success signals

- Adding a provider touches one new file and no existing behavior.
- A run against a provider that is down produces that provider's rows missing and everything else
...
- Politeness is not tunable downward below a floor: per-source rate limiting, backoff, jitter on
  ...
  slow.
- Provider result ceilings are facts to be worked around, not ignored: Realtor.com caps at 10,000
  per query.
...
- Deciding whether two providers' rows describe the same house (feat-006).
- Turning a saved search's areas into provider-shaped queries (feat-004), and orchestrating a run
  across providers (feat-003). This feature answers one query against one provider.
